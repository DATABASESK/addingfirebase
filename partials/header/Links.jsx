...
const Links = ({ isMobile }) => {
  ...
  if (isMobile) {
    return (
      <div className="flex flex-col h-full justify-between items-center  text-[#c4c2c7] p-2 gap-2 overflow-hidden">
    ...
    <div className="flex mt-[8px] text-[#c4c2c7] max-[990px]:hidden">
      {links.map((link, index) => (
        <Link
          ...
          key={link}
          className={`${index === 0 ? "ml-6" : "ml-6"} ${(pathname === "/" ? "home" : pathname).includes(link.toLowerCase()) ? "text-white" : ""}`}
        >
          {link}
        </Link>
      ))}
