...
export const getTopNetflixSeriesGlobal = async (page = 1) => {
  ...
  const watchRegion = "US"; // Change to another country code if needed

  // Blockbuster Genre IDs for Sci-Fi, Action, Time Travel, Mystery
  const genreIds = [878, 28, 10765, 9648]; // Sci-Fi (878), Action (28), Time Travel (10765), Mystery (9648)

  const url = `https://api.themoviedb.org/3/discover/tv?sort_by=popularity.desc&first_air_date.lte=${today}&with_watch_providers=${netflixProvider}&watch_region=${watchRegion}&include_adult=true&page=${page}&with_genres=${genreIds.join(',')}&api_key=38248c047fd8ef9b0a7e25d40651e870`;
