// Fetch Recent Released Tamil Movies Available on OTT Platforms
export const getRecentTamilMovies = async (page = 1) => {
  const today = new Date().toISOString().split("T")[0]; // Get today's date in YYYY-MM-DD format

  // OTT Provider IDs (TMDB watch provider IDs)
  const ottProviders = [8, 9, 119, 122, 224, 237, 339]; // Netflix, Amazon Prime, Zee5, Aha, SunNXT, SonyLIV, JioCinema
  ...
  const url = `https://api.themoviedb.org/3/discover/movie?language=ta-IN&region=IN&with_original_language=ta&sort_by=release_date.desc&release_date.lte=${today}&with_watch_providers=${ottProviders.join(
    "|"
  )}&watch_region=IN&page=${page}&38248c047fd8ef9b0a7e25d40651e870`;
