...
export const getMcuMovies = async (page = 1) => {
  ...
  const mcuMoviesTitles = [
    ...
    "Doctor Strange", "Thor: Ragnarok", "Ant-Man and the Wasp",
    "Avengers: Infinity War", "Avengers: Endgame", "Spider-Man: Far From Home",
    "Black Panther", "Spider-Man: Homecoming", "Captain Marvel",
    "Avengers: Endgame", "Black Widow", "Shang-Chi and the Legend of the Ten Rings",
    ...
    "The Marvels", "Blade", "Fantastic Four",  // Future Movies
  
    // **Deadpool and X-Men Related Movies**
    "Deadpool", "Deadpool 2", "Logan", "X-Men", "X-Men: Days of Future Past", 
    "X-Men: Apocalypse", "X-Men: Dark Phoenix", "The Wolverine", 
    "X-Men Origins: Wolverine", "X2: X-Men United", "X-Men: The Last Stand",
    "The New Mutants", "Deadpool 3", "Wolverine (2024)",  // Upcoming Deadpool & Wolverine Films
  
    // **DC Extended Universe (DCEU) Movies**
    "Man of Steel", "Batman v Superman: Dawn of Justice", "Wonder Woman", 
    "Justice League", "Aquaman", "Shazam!", "Birds of Prey", "The Suicide Squad", 
    "Wonder Woman 1984", "Zack Snyder's Justice League", "The Batman", 
    "Black Adam", "Shazam! Fury of the Gods", "Aquaman and the Lost Kingdom", 
    "The Flash", "Blue Beetle", "Joker", "Joker: Folie a Deux", // Other DC Films
    
    // **Other Notable DC Films**
    "Superman: The Movie", "Batman: The Movie (1966)", "Batman (1989)", "Batman Returns",
    "Batman Forever", "Batman and Robin", "The Dark Knight", "The Dark Knight Rises", 
    "Wonder Woman (2017)", "Justice League (2017)", "The Suicide Squad (2021)",
    "Constantine", "V for Vendetta", "The Green Lantern", "Watchmen", "The Losers"
  ];

  try {
    const movies = [];
    
    for (const title of mcuMoviesTitles) {
      const url = `https://api.themoviedb.org/3/search/movie?query=${encodeURIComponent(title)}&api_key=${apiKey}&page=${page}`;
      ...
      if (data.results.length > 0) {
        movies.push(data.results[0]); // Assuming the first result is the correct movie
      }
    }
