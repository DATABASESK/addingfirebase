...
const Card = ({ data, index, loading, hidden, type }) => {
  ...
  const mediaType = data?.media_type || type || (data?.first_air_date ? "tv" : "movie");

  return (
    <div className="aspect-[9/14] rounded-2xl cursor-pointer mb-2 relative">
      <Link href={`/watch/${data?.id}?media_type=${mediaType}`} className={styles.wrapper}>
        <Image
          src={
            data?.poster_path
        ...
        <div
          className={`${styles.info} bottom-2 left-0 right-0 absolute text-xs font-medium flex flex-wrap items-center justify-center gap-[.3rem] z-[7] opacity-0`}
        >
